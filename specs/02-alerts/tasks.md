...
- [x] **0.1** Implement `app/models/base_model.py` with `AuditMixin` class — `id`, `deleted`, `created_at`, `updated_at` columns
- [x] **0.2** Refactor `app/models/weather_forecast.py` to inherit from `AuditMixin` — remove `id`, `created_at`, `updated_at` from the model (now provided by mixin)
- [x] **0.3** Update `app/repositories/base.py`:
...
- [x] **0.4** Rebuild DB locally to pick up `AuditMixin` changes: `docker compose down -v && docker compose up -d`
- [x] **0.5** Add `NotificationChannel` enum to `app/models/enums.py`
- [x] **0.6** Add `redis` to project dependencies (`pyproject.toml` / `requirements.txt`) and implement `app/core/redis_client.py`:
  ```python
  import redis
  ...
  Note: the correct import path is `from app.config import settings` (not `app.core.config`).

---

## Phase 1 — Models and Migration `[P]`

- [x] **1.1** `[P]` Implement `app/models/alert.py` — inherits `AuditMixin`, `field_id` as logical UUID reference, `notification_channel` + `contact` columns, indexes → satisfies US-1.1
- [x] **1.2** `[P]` Implement `app/models/notification.py` — inherits `AuditMixin`, FK to alerts with `ON DELETE RESTRICT` (not CASCADE — app exclusively soft-deletes), indexes → satisfies US-2.1
- [x] **1.3** Register `Alert` and `Notification` in `app/models/__init__.py` so Alembic autogenerate detects them
- [x] **1.4** Generate a single Alembic migration covering all schema changes in this phase:
...
- [x] **2.1** `[P]` Implement `app/repositories/alert_repository.py` — async, extends `BaseRepository[Alert]`:
...
- [x] **2.2** `[P]` Implement `app/repositories/notification_repository.py` — async, extends `BaseRepository[Notification]`:
...
- [x] **2.3** `[P]` Implement `app/repositories/alert_sync_repository.py` — sync, extends `SyncBaseRepository[Alert]`:
...
- [x] **2.4** `[P]` Implement `app/repositories/notification_sync_repository.py` — sync, extends `SyncBaseRepository[Notification]`:
...
- [x] **2.5** `[P]` Implement `app/repositories/weather_forecast_sync_repository.py` — sync, extends `SyncBaseRepository[WeatherForecast]`:
...
- [x] **3.1** `[P]` Implement `app/schemas/alert.py`:
...
- [x] **3.2** `[P]` Implement `app/schemas/notification.py`:
  - `NotificationResponse` — `id`, `alert_id`, `target_date`, `probability`, `status`, `triggered_at`
- [x] **3.3** Implement `app/schemas/common.py`:
...
- [x] **4.1** `[P]` Implement `app/services/alert_service.py` — orchestrates repository calls:
  ...
  - `delete(alert_id, session) → None`:
    ...
    → satisfies US-1.3
- [x] **4.2** `[P]` Implement `app/services/notification_service.py`:
...
- [x] **5.1** `[P]` Implement `app/routers/alerts.py` with prefix `/api/v1/alerts`:
...
- [x] **5.2** `[P]` Implement `app/routers/notifications.py` with prefix `/api/v1/alerts`:
...
- [x] **5.3** Register both routers in `app/main.py` with prefix `/api/v1`
...
- [x] **6.1** Implement `app/tasks/evaluate_alerts.py` → satisfies US-3.1:
...
- [x] **6.2** Implement `app/tasks/deliver_notification.py` → satisfies US-3.1:
...
- [x] **6.3** Register `evaluate_alerts` in `app/core/celery_app.py` beat schedule using `settings.alert_eval_interval_seconds`
- [x] **6.4** Update `app/core/celery_app.py` `include` to list both task modules explicitly:
...
- [x] **7.1** Verify migrations run cleanly against local PostgreSQL
- [x] **7.2** Run seed script and confirm evaluator creates notifications end-to-end (Docker Compose)
- [x] **7.3** `[P]` Write integration tests for alert endpoints → satisfies US-1.1, US-1.2, US-1.3:
...
- [x] **7.4** `[P]` Write integration tests for notification endpoint → satisfies US-2.1:
...
- [x] **7.5** `[P]` Write unit tests for evaluator logic → satisfies US-3.1:
